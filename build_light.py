...
function selectRegion(v,e){
  document.querySelectorAll(".region-chip").forEach(function(c){c.classList.remove("active")});if(e)e.classList.add("active");
  var map={all:"全部",mainland:"大陆",hongkong:"香港",macau:"澳门",taiwan:"台湾",overseas:"海外"};_rf=map[v]||"全部";renderResults();
}
function selectType(v,e){
  document.querySelectorAll(".type-chip").forEach(function(c){c.classList.remove("active")});if(e)e.classList.add("active");
  var map={all:"全部",stamp:"邮票",sheet:"小型张",cover:"实寄封",postcard:"明信片"};_tf=map[v]||"全部";renderResults();
}
function toggleSort(){
  var b=document.querySelector(".sort-btn");if(!b)return;
  var o=["默认排序","价格升序","价格降序","年份排序"];var i=o.indexOf(b.textContent.trim());b.textContent=o[(i+1)%o.length];renderResults();
}
